...
export const useVotePhase = () => {
  ...
  const opponents = useSelector(selectPlayers).filter((player: UserState) => player.id !== myUserId);
  const finalVotes = useSelector(selectFinalVotes);
  const unvotedOpponents = opponents.filter((player: UserState) => !finalVotes[player.id as UserId]).map(player => player.nickname)
  .join(', ');
